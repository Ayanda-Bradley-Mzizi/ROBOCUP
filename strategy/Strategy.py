...
class Strategy():
    ...
    def DetermineGameState(self):
        # Looks at where the ball is and who has possession

        # get the balls x-pos to see which half its in
        ball_x = self.ball_2d[0]

        # Check who is closer to the ball (thats who probably has possession)
        # If our closest player is closer than their closest player, we likely have the ball
        we_have_possession = self.min_teammate_ball_dist < self.min_opponent_ball_dist

        # Now decide what formation we need based on ball position and possession
        if ball_x <= -1:
            # Ball is in our side
            if we_have_possession:
                # We have the ball in our half - build up
                return "build_up"
            else:
                # They have the ball in our half - defend
                return "defensive"
        elif ball_x >= 0:
            # Ball is in their side
            if we_have_possession:
                # We have the ball near their goal - attack
                return "attacking"
            else:
                # They have the ball but theyre deep - press
                return "pressing"

        # below is for if we want to section field into 3 parts (defensive, midfield, attacking)
        """
        else:
            # Ball is in the middle
            if we_have_possession:
                # We're moving the ball forward - transition to attack
                return "attacking"
            else:
                # They're moving the ball - get ready to defend
                return "defensive"
        """
